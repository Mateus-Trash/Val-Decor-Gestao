  ...
  getComparativoMensal: protectedProcedure
    .input(
      z.object({
        dataInicio: z.coerce.date(),
        dataFim: z.coerce.date(),
        dataInicioAnterior: z.coerce.date(),
        dataFimAnterior: z.coerce.date(),
      })
    )
    .query(async ({ input }) => {
      const db = await getDb();
      ...
      const inicioAtual = input.dataInicio;
      const fimAtual = input.dataFim;
      const inicioAnterior = input.dataInicioAnterior;
      const fimAnterior = input.dataFimAnterior;
